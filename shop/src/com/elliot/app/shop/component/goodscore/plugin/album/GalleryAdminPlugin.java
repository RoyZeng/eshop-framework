...
import com.elliot.framework.image.ImageRuntimeException;
...
public class GalleryAdminPlugin extends DefaultAlbumPlugin implements IOnSettingInputShow,IGoodsDeleteEvent{
	...
	protected void proessPhoto(String[] picnames,Map goods) {
		...
		if (picnames == null) {
		//	goods.put("image_file",  ParamSetting.IMG_SERVER_DOMAIN+"/images/no_picture.jpg");
		//	goods.put("image_default", ParamSetting.IMG_SERVER_DOMAIN+"/images/no_picture.jpg");
			return;
		}
...
//		imgsString  =UploadUtil.replacePath(imgsString);
		imgsString= imgsString.replaceAll(ParamSetting.IMG_SERVER_DOMAIN+EopContext.getContext().getContextPath(), ParamSetting.FILE_STORE_PREFIX);
		goods.put("image_file", imgsString);
		String image_default = (String)goods.get("image_default");
		if(image_default!=null){
			image_default= image_default.replaceAll(ParamSetting.IMG_SERVER_DOMAIN+EopContext.getContext().getContextPath(), ParamSetting.FILE_STORE_PREFIX);
